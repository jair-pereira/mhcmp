...
set_n   = [  25,      25,     25,    200,    200]
set_g   = [0.11,    0.39,   0.99,   0.87,   0.90]
set_a   = [3.74,    1.53,   0.92,   3.49,   1.63]
...
tmp = ",".join(map(str, set_g))
file.write(f"gamma:{tmp}\n")
...
for problem in suite:
    ...
    for rep in range(max_rep):
        for i in range(restart):
            ...
            ## run method ##
            result = ffa(problem=problem,

                         maxnfe=maxnfe,

                         n=set_n[i], alpha=set_a[i], gamma=set_g[i],
