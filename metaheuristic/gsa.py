...
def gsa(problem, maxnfe, n, g0, alpha, seed, file=None):
    ...
    ## kbest ##
    kbest1 = np.linspace(n, 1, int(max_iteration)) # linear, slow
    kbest2 = [int(np.ceil(n*np.e**(-alpha*i/max_iteration))) for i in range(max_iteration)] #exp fast
    kbest3 = map(int, [(k1+k2)/2 for k1, k2 in zip(kbest1, kbest2)]) # exp+linear
    ...
    while nfe+n <= maxnfe and not problem.final_target_hit:
        ...
        R  = myrng.random((n, problem.dimension))
        Pv = [Pacceleration[i] + Pv[i]*R[i] for i in range(n)]
